...
football-data.org Bundesliga team calendars are generated by the same import
job. Configure `FOOTBALL_DATA_API_KEY` in the local shell or as a GitHub Actions
secret before running the production import. The job fetches Bundesliga teams
and scheduled matches, writes one `.ics` file per team, and updates
`output/football-data-bl1-teams.json` so the web app can list the team feeds.
