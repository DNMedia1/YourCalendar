...
Recommended sport calendars are generated by the same import job. The import
uses a grouped provider registry for football, boxing, ice hockey, baseball,
basketball, American football, Formula 1, golf, Tour de France, darts, chess,
esports and UFC. Team leagues create one `.ics` file per discovered team;
non-team sports create competition feeds when a safe provider is enabled. The
web app reads `output/sport-feeds.json` to list generated feeds by group and
subgroup.

Configure provider secrets in the local shell or as GitHub Actions secrets
before running production imports. `FOOTBALL_DATA_API_KEY` is required for the
football-data.org Bundesliga path. `THESPORTSDB_API_KEY` is optional and
defaults to the public demo key `123`. `PANDASCORE_API_KEY` is reserved for a
future esports importer and is not enabled until a concrete free-tier scope is
verified.
...
can be started manually and runs daily at 03:00 Europe/Berlin during CEST
through a UTC GitHub Actions cron. It uploads feed,
